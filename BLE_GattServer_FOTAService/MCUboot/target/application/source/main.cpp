                                               ...
                                               mbed::Span<const uint8_t> buffer) override
    {
        ...
        if (buffer[0] == FOTAService::FOTA_COMMIT) {
            ...
            if (err) {
                ...
                fota_service.notify_status(FOTAService::FOTA_STATUS_INSTALLATION_FAILURE);
                return AUTH_CALLBACK_REPLY_ATTERR_UNLIKELY_ERROR;
            } else {
...
{
    mbed_trace_init();
    mbed_trace_include_filters_set("MAIN,FOTA,MCUb,BL");
    ...
    tr_info("Demo complete, restarting to apply update");
    demo.disconnect(ble::local_disconnection_reason_t::POWER_OFF);
    /* Dispatch lingering events */
